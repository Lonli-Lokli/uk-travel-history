...
import { FEATURE_KEYS, type FeatureFlagKey, type FeaturePolicy, TIERS } from '@uth/features';
...
function FeatureAccessBadge({ hasAccess, isEnabled, minTier, userTier }: FeatureAccessBadgeProps) {
  if (!isEnabled) {
    return (
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {minTier !== 'anonymous' && (
        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${tierColors[minTier as 'free' | 'premium']}`}>
          Requires: {minTier}
        </span>
      )}
    </div>
...
export const StatusPageClient = observer(({ featurePolicies, isSupabaseAvailable }: StatusPageClientProps) => {
    ...
    <div className="max-w-6xl mx-auto px-4 py-4 md:py-6 space-y-4 md:space-y-6">
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-4 md:p-8">
        ...
        <div className={`mb-4 p-3 rounded-lg border flex items-center gap-2 ${
          ...
          <div className={`text-xs md:text-sm ${
            ...
            <strong>Data Source:</strong> {isSupabaseAvailable ? 'Database (live policies)' : 'Using default fallback values'}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 bg-slate-50 rounded-lg border border-slate-200">
          <div className="flex items-center gap-3">
            <UIIcon iconName="user" className="h-5 w-5 text-slate-600 flex-shrink-0" />
            <div>
              <div className="text-xs md:text-sm text-slate-600">Your Current Tier</div>
              <div className="text-sm md:text-base font-semibold text-slate-900">{tierLabels[userTier]}</div>
            </div>
          </div>
          {!isAuthenticated && (
        ...
        >
          ...
          <div className="space-y-4">
            {features.map((feature) => {
              ...
              const hasAccess = monetizationStore.hasFeatureAccess(feature.key);
      ...
      <div className="bg-blue-50 rounded-lg border border-blue-200 p-4 md:p-6">
        <h3 className="text-xs md:text-sm font-semibold text-blue-900 mb-3 flex items-center">
          <UIIcon iconName="info-circle" className="h-4 w-4 mr-2 flex-shrink-0" />
          Feature Access Information
        </h3>
        <div className="text-xs md:text-sm text-blue-800 space-y-2">
          <p>
            <strong>Your Access:</strong> Features showing "You have access" are available to you with your current tier.
          </p>
          <p>
            <strong>Tier Requirements:</strong> Each feature shows the minimum subscription tier required for access.
          </p>
          <p>
            <strong>Anonymous:</strong> Available to everyone without signing in
          </p>
          <p>
