...
export const SummaryCards = observer(() => {
  ...
  const hasILRStatus = validation && hasContinuousLeave && summary.remaining180LimitToday !== null;
  const isEligible = validation?.status === 'ELIGIBLE';

  const gridColsClass = hasILRStatus ? 'sm:grid-cols-1 lg:grid-cols-2' : 'sm:grid-cols-1';
          ...
          stats={[
            {
              ...
              label: 'Total Trips',
              tooltip: 'Total number of trips outside the UK that you have recorded.'
            },
            {
              value: summary.completeTrips,
              label: 'Complete',
              variant: 'success',
              tooltip: 'Number of trips with both departure and return dates filled in.'
            },
            {
              value: summary.incompleteTrips,
              label: 'Incomplete',
              variant: summary.incompleteTrips > 0 ? 'warning' : 'default',
              tooltip: 'Number of trips missing either departure or return date. Complete these to calculate accurate ILR eligibility.'
            },
            {
              value: summary.totalFullDays,
              label: 'Days Outside',
              tooltip: 'Total full days spent outside the UK. Full days exclude your departure and return days (as per UK Home Office guidance).'
            },
            {
              value: summary.continuousLeaveDays!,
              label: 'Days in UK',
              variant: summary.hasExceededAllowedAbsense ? 'warning' : 'success',
              tooltip: 'Total days you have been physically present in the UK during your continuous residence period, calculated from your vignette entry or visa start date.'
            },
            {
              value: summary.maxAbsenceInAny12Months ?? '—',
              label: 'Max 12mo',
              variant: summary.hasExceededAllowedAbsense ? 'warning' : 'default',
              tooltip: 'The maximum number of days you were absent in any single rolling 12-month period. This must not exceed 180 days to maintain continuous residence.'
            },
          ]}
              ...
              ? [
                  {
                    ...
                    label: 'Eligible Date',
                    tooltip: 'The date when you become eligible to apply for Indefinite Leave to Remain (ILR) based on your continuous residence period.'
                  },
                  {
                    value: summary.daysUntilEligible !== null
                      ? summary.daysUntilEligible <= 0
                        ? '✓ Eligible'
                        : `${summary.daysUntilEligible}d`
                      : '—',
                    label: 'Days Until ILR',
                    variant: summary.daysUntilEligible !== null && summary.daysUntilEligible <= 0 ? 'success' : 'default',
                    tooltip: summary.daysUntilEligible !== null && summary.daysUntilEligible > 0
                  ...
                  },
                  {
                    ...
                    variant:
                      summary.remaining180LimitToday !== null && summary.remaining180LimitToday < 30
                        ? 'warning'
                        : 'success',
                    tooltip: 'Days remaining before reaching the 180-day absence limit in the current rolling 12-month period. You can be absent for up to 180 days in any consecutive 12 months.'
                  },
                  {
              ...
              ? [
                  {
                    ...
                    label: 'ILR Status',
                    variant: 'warning',
                    tooltip: 'You are currently not eligible to apply for Indefinite Leave to Remain. See the reason below and the detailed validation card for more information.'
                  },
                  {
                    value:
