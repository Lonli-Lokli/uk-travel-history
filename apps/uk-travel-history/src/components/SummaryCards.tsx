...
import { CompoundStatCard, StatItem } from '@uth/ui';
import { FC } from 'react';
import { ILRSummary, ILRValidationResult } from '@uth/rules';
...
export const SummaryCards = observer(() => {
  ...
  return (
    <div className={`grid grid-cols-1 ${gridColsClass} gap-2 mb-3`}>
      ...
      {hasContinuousLeave ? (
        <ILRHistoryCard summary={summary} />
      ) : (
        <TravelHistoryCard summary={summary} />
      )}
...
const ILRHistoryCard: FC<{ summary: ILRSummary }> = ({ summary }) => {
  return (
    <CompoundStatCard
...
const ILRStatusCard: FC<{
  isEligible: boolean;
  summary: ILRSummary;
  validation: ILRValidationResult;
}> = ({ isEligible, summary, validation }) => {
  const ilrStats: StatItem[] =
...
const TravelHistoryCard: FC<{ summary: ILRSummary }> = ({ summary }) => {
  return (
    <CompoundStatCard
      icon="airplane"
