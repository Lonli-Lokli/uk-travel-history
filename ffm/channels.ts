 ...
 */
export type ChannelPayload<T> = T extends ChannelDefinition<any, infer P, any> ? P : never;

/**
 * Extract channel name from a channel definition
 */
export type ChannelName<T> = T extends ChannelDefinition<infer N, any, any> ? N : never;

/**
 * Extract direction from a channel definition
 */
export type ChannelDirection<T> = T extends ChannelDefinition<any, any, infer D> ? D : never;

/**
 * Filter channels by direction
 */
export type ChannelsByDirection<
  TChannels extends Record<string, ChannelDefinition<string, unknown, MessageDirection>>,
  TDirection extends MessageDirection
> = {
  [K in keyof TChannels as ChannelDirection<TChannels[K]> extends TDirection ? K : never]: TChannels[K]
};
